  ...
  const auditRules = [
    {
      ...
      name: 'Meta Title',
      weight: '15%',
      purpose: 'Ensures the page has a valid title tag containing the primary target phrase near the beginning (within the first 30 characters) and adheres to optimal length guidelines (50–65 characters) for maximum SERP click-through rate.',
      dataExtracted: '<title> tag text string and character count (len(title)).',
      matchingLogic: 'Exact / Start Match (first 30 chars), Contains Match (substring), or Topical Match (stemmed + geo-normalized token cover ≥80%).',
      scoringContribution: '15 Points (Title area weight: +80 start match, +55 contains match, +50 topic match, +20 bonus for 50-65 chars).',
      passCriteria: 'Title is present AND target phrase is in title AND title length is between 50–65 characters.',
      failCriteria: 'Title tag missing OR target phrase completely absent from title.',
      recommendedAction: 'Add "[target phrase]" to the meta title, ideally at the start.',
      passExample: 'Title: "Loft Conversions South London | Expert Home Extensions" (Target: "Loft Conversions South London")',
