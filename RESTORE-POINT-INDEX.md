...
|---|---|---|---|---|---|
| v2.13-stable | `v2.13-stable-shared-domain-foundation` | `[AUTO]` | 07-09-2026 18:05 | Completed Site Registry → Website Manager Shared-Domain Foundation: Secure backend domain write path, anon SELECT-only Supabase RLS, Website Manager domain_id Master UUID link (14/14 sites mapped, 180 page configs preserved), SEO Context bridge operational, automatic Active/Development domain → Website Manager shell creation with 'NEW FROM SITE REGISTRY' / 'SETUP REQUIRED' state, duplicate prevention via partial unique index, idempotent reconciliation. | **Current** |
