...
|---|---|---|---|---|---|
| v2.21-stable | `v2.21-stable-classification-engine-hostname-isolation-fix` | `[AUTO]` | 10-09-2026 10:20 | Isolated URL pathname/slug from domain hostname in packageExtractor.js automatic exclusion rules. Fixed false positive where keywords like 'search' in domain names (e.g. thesearchequation.com) falsely excluded all non-homepage pages. Restored proper classification for The Search Equation (1 Hub, 29 Landing, 4 Topical, 74 Article, 5 Excluded) with zero regression on existing sites (Ascent Builders, Bathroom Upgrades, HF4You, Civion). | **Current** |
| v2.20-stable | `v2.20-stable-website-manager-realtime` | `e809ddd` | 10-09-2026 10:00 | Real-Time Multi-User Synchronization operational in TSE Website Manager: Supabase Realtime Broadcast layer (<50ms pub/sub) synchronizes active administrative sessions (Mac & Deb) automatically without browser reload across website tiles, page configurations (target phrases, page types, priority), package syncs, page audits, and internal linking recommendations. Self-echo immunity, loop prevention, unmount cleanup, and SQLite/Express single-source-of-truth persistence verified live. | Superseded |
| v2.12-stable-before-realtime | `v2.12-stable-before-website-manager-realtime` | `2396766` | 10-09-2026 09:50 | Confirmed stable baseline restore point of TSE Website Manager prior to introducing Supabase Realtime Broadcast multi-user synchronization. Authoritative SQLite/Express backend, WordPress Exporter & REST API sync/push, Magento 2 sync/auth, W3 page classification & natural sorting, and W4 page auditor all verified operational. | Superseded |
| v2.17-stable | `v2.17-stable-wordpress-structural-rules-and-classification` | `d454c14` | 09-09-2026 17:35 | Sitewide generic WordPress page classification rules: Absolute Homepage rule (Homepage / root URL → Hub / Priority 1 sitewide), Structural Landing rules (/services/, /locations/, /areas/, /areas-we-cover/ & child pages → Landing / Priority 2), sitewide natural alphanumeric column sorting across W3 Manage Pages, route state persistence on refresh, and WordPress REST API live Push 401 resolution. | Superseded |

| v2.14-stable | `v2.14-stable-backlinks-phase-2a-foundations` | `[AUTO]` | 07-09-2026 18:18 | Backlinks Phase 2A Foundations Completed: 530 backlink targets linked to Master Domain UUIDs (0 unmatched / 0 ambiguous), Master Source Database (1,372 sources) preserved with operational usage & 2-client deduplication checks, Target-Page Anchor Pool schema/foundation implemented (1,637 anchors: 1,126 historical used anchors preserved + 511 available target phrases), Website Manager SEO Context bridge active, 126 Master Domains & 14 WM sites preserved. | Superseded |

| v2.13-stable | `v2.13-stable-shared-domain-foundation` | `701baf4` | 07-09-2026 18:05 | Completed Site Registry → Website Manager Shared-Domain Foundation: Secure backend domain write path, anon SELECT-only Supabase RLS, Website Manager domain_id Master UUID link (14/14 sites mapped, 180 page configs preserved), SEO Context bridge operational, automatic Active/Development domain → Website Manager shell creation with 'NEW FROM SITE REGISTRY' / 'SETUP REQUIRED' state, duplicate prevention via partial unique index, idempotent reconciliation. | Superseded |
| v2.12-stable | `v2.12-stable-site-registry-before-backlinks-phase-2a` | `3c31682` | 07-09-2026 17:31 | Confirmed Stable Site Registry: 126 Master Domain records in public.domains, redesigned Domains table with standard TSE UI font (+1px), duplicate URLs removed, widened table layout, Domain Control Page (/domains/:id), Backlinks Phase 1 & 2 audit completed. | Superseded |
